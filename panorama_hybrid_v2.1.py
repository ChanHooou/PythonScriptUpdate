...
class GlobalOptimizationStitcher:
    """전역 최적화 파노라마 스티칭 (v2.1 - 우측 하단 0,0 기준 좌표계)

    v2.1 좌표계 (rotate_output=True 시):
    - 우측 하단: (0, 0) - 원점
    - 좌측 하단: (0, BUILDING_WIDTH)
    - 우측 상단: (BUILDING_HEIGHT, 0)
    - 좌측 상단: (BUILDING_HEIGHT, BUILDING_WIDTH)
    - X축: B센서 값 (하단 0 -> 상단 BUILDING_HEIGHT)
    ...
    def __init__(self,
        ...
        # 트렌드 기반 보정을 위한 변수 (v2.1: median 기반)
        ...
        print(f"\n{'='*60}")
        print("Global Optimization Panorama Stitcher v2.1")
    ...
    def calculate_sensor_offset(self, idx: int) -> Tuple[int, int]:
        ...
        if self.vertical_sensor == "F":
            ...
            curr_corrected_vertical = prev_corrected_vertical + vertical_diff
            self.corrected_vertical_values.append(curr_corrected_vertical)
            # v2.1: B센서는 0 기준 정방향 (rotate 후 X축)
            dy = -int(vertical_diff * self.PIXEL_PER_CM_Y)
        ...
        if self.horizontal_sensor == "L":
            ...
            # v2.1: L센서는 building_width 기준 역방향 (rotate 후 Y축, 우측 0 기준)
    ...
    def calculate_coverage_range(self) -> Tuple[int, int, int, int]:
        ...
        if self.vertical_sensor == "F":
            ...
            # B센서: 0 기준 정방향 (v2.1 - rotate 후 X축으로 사용, 우측 하단 0,0 기준)
            y_min = min(b_values)
        ...
        if self.horizontal_sensor == "L":
            ...
            # L센서: building_width 기준 역방향 (v2.1 - rotate 후 Y축으로 사용, 우측 0 기준)
            x_min = self.building_width - max(l_values) - self.IMAGE_REAL_WIDTH
            x_max = self.building_width - min(l_values)
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("\nUsage: python panorama_hybrid_v2.1_full.py <image_folder> [output_folder] [options...]")
        print("\nBasic Example:")
        print("  python panorama_hybrid_v2.1_full.py ./images")
        print("\nWith sensor filtering and rotation:")
        print("  python panorama_hybrid_v2.1_full.py ./images ./output BL backward 0 0.3 3 1620 810 125 87 20 True")
        print("\nArguments:")
        ...
        print("\n✨ v2.1 NEW FEATURES:")
        print("  - 우측 하단 (0,0) 기준 좌표계 (rotate_output=True 권장)")
        print("  - X축: B센서 값 (하단 0 -> 상단 BUILDING_HEIGHT)")
        print("  - Y축: BUILDING_WIDTH - L센서 값 (우측 0 -> 좌측 BUILDING_WIDTH)")
        print("  - JPG 출력만 지원 (PNG 제거)")
        print("  - 파일명 간소화: result_Xmin,Ymin-Xmax,Ymax.jpg")
